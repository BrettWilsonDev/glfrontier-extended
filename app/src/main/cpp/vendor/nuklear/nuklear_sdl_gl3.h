 ...
 * Modernized SDL + OpenGL backend (Desktop GL 3.3 Core + Emscripten WebGL2)
 * No Glad dependency on Emscripten path.
 */
...
#include <SDL.h>
    ...
    #include <GLES3/gl3.h>
#else
    #include "glad/glad.h"   // Only used on desktop builds
#endif
...
/* Shader version selection for modern desktop vs Emscripten */
...
NK_API void nk_sdl_device_create(void)
{
    ...
    /* VAO + VBO setup (modern core / ES3 compatible) */
...
/* The render function stays almost identical — it's already modern */
NK_API void nk_sdl_render(enum nk_anti_aliasing AA, int max_vertex_buffer, int max_element_buffer)
{
    struct nk_sdl_device *dev = &sdl.ogl;
    ...
    SDL_GL_GetDrawableSize(sdl.win, &display_w, &display_h);

    ortho[0][0] /= (GLfloat)width;
    ortho[1][1] /= (GLfloat)height;

    scale.x = (float)display_w / (float)width;
    scale.y = (float)display_h / (float)height;
...

    {
        ...
        /* WebGL2 does not support glMapBufferRange — use client-side buffers instead */
        void* vertices = malloc((size_t)max_vertex_buffer);
        void* elements = malloc((size_t)max_element_buffer);
...
/* Rest of the functions (init, font stash, event handling, shutdown, handle_grab) are identical to your original. */
...
/* Call this after a window resize to rebuild the font atlas at the new scale.
 * Pass the font .ttf data + byte length, and the new desired pixel size.
 * If ttf_data is NULL, falls back to the Nuklear built-in bitmap font. */
NK_API void nk_sdl_reload_font(const void *ttf_data, int ttf_len, float font_size)
{
    struct nk_font_atlas *atlas;
    struct nk_font *font = NULL;

    /* free the old texture and atlas */
    ...
    if (ttf_data && ttf_len > 0)
        font = nk_font_atlas_add_from_memory(atlas, (void*)ttf_data, ttf_len, font_size, NULL);
    nk_sdl_font_stash_end();   /* bakes + uploads new texture */
...
#endif /* NK_SDL_GL3_IMPLEMENTATION */
