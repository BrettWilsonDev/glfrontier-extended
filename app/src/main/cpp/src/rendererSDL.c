...
static void change_vidmode()
{
	if (sdlWindow == NULL)
	{
		...
		if (!sdlWindow)
		{
			log_printf("Window creation failed: %s\n", SDL_GetError());
			SDL_Quit();
			exit(-1);
		}
		...
		if (!sdlRenderer)
		{
			log_printf("Renderer creation failed: %s\n", SDL_GetError());
			SDL_Quit();
			exit(-1);
		}
		...
		if (!sdlTexture)
		{
			log_printf("Texture creation failed: %s\n", SDL_GetError());
			SDL_Quit();
			exit(-1);
		}
...
void Screen_ToggleFullScreen()
{
	...
	if (bInFullScreen)
	{
		...
		{
			log_printf("Could not set window to full screen: %s", SDL_GetError());
			SDL_DestroyWindow(sdlWindow);
			SDL_Quit();
			return;
		}
	...
	else
	{
		...
		{
			log_printf("Could not set window to windowed mode: %s", SDL_GetError());
			SDL_DestroyWindow(sdlWindow);
			SDL_Quit();
			return;
		}
...
/* Letterbox helpers — SDL renderer always fills the full window,
   so offsets are 0 and game dims equal window dims. */
int Screen_GetGameOffsetX(void) { return 0; }
int Screen_GetGameOffsetY(void) { return 0; }
int Screen_GetGameWidth(void)   { return screen_w; }
int Screen_GetGameHeight(void)  { return screen_h; }
void call_update_letterbox(void) { /* no letterbox in SDL renderer */ }
