...
static bool lb_mode = true;   /* default: keep aspect ratio */

static void update_letterbox(void)
{
    if (!lb_mode)
    {
        /* Fill the full window — no bars */
        lb_x = 0;
        lb_y = 0;
        lb_w = screen_w;
        lb_h = screen_h;
    ...
    /* Original aspect-correct letterbox */
    float aspect = (float)GAME_W / (float)GAME_H;   /* 4/3 */
...
void draw_rect_debug(int x, int y, int w, int h, unsigned char r, unsigned char g, unsigned char b)
{
    push_screen_ortho();
    set_color3ub(r, g, b);
    batch_begin(GL_LINE_LOOP);
...
static void draw_on_top_of_screen(void)
{
	...
	{
		// fn buttons touch regions
		unsigned char debug_colors[][3] = {
			{255, 0,   0},
			{0,   255, 0},
			{0,   0,   255},
			{255, 255, 0},
		...
		for (int i = 0; i < 15; i++)
			draw_rect_debug(fn_buttons[i].x, fn_buttons[i].y,
							fn_buttons[i].width, fn_buttons[i].height,
							debug_colors[i][0], debug_colors[i][1], debug_colors[i][2]);
	}
