...
static void play_music(int track)
{
	...
	snprintf(buf, sizeof(buf), "assets/music/%02d.ogg", track);
...
void Audio_Init(void)
{
	...
	{
		snprintf(filename, sizeof(filename), "assets/sfx/sfx_%02d.wav", i);
		// if (SDL_LoadWAV(filename, &desiredAudioSpec, &sfx_buf[i].buf,
		// 				&sfx_buf[i].buf_len) == NULL)
		if (SDL_LoadWAV(filename, &desiredAudioSpec, &sfx_buf[i].buf, (Uint32 *)&sfx_buf[i].buf_len) == NULL)
		{
		...
		if (i == 19)
			sfx_buf[i].loop = SND_FREQ; /* loop to about 0.5 sec in */
		else if (i == 23)
			sfx_buf[i].loop = 0;
		else
			sfx_buf[i].loop = -1;
