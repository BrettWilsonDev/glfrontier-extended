...
static void change_vidmode()
{
	...
	window = SDL_CreateWindow(
		...
		screen_w,				// width
		screen_h,				// height
		window_flags			// flags (fullscreen, OpenGL, etc.)
	);
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, SCR_TEX_W, SCR_TEX_H, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
...
void Nu_QueueDrawStr()
{
	...
	strncpy(reinterpret_cast<char*>(queued_strings[queued_string_pos].str), GetReg(REG_A0) + STRam, 64);
...
void Screen_ToggleRenderer()
{
	...
	use_renderer = static_cast<RENDERERS>((static_cast<int>(use_renderer) + 1) % R_MAX);
}
...
static void draw_control_panel()
{
	...
	/* this is a big  hack to make starsystem names
	...
	/* hack hack hack what a pile of e this is */
	push_ortho();
...
static void lighting_on(float light_vec[4], int rgb444_light_col, int rgb444_extra_col, int rgb444_obj_col)
{
	...
	else
	{
		...
		glLightiv(GL_LIGHT0, GL_DIFFUSE, (const GLint *)light_col);
		glLightiv(GL_LIGHT0, GL_AMBIENT, (const GLint *)obj_col);
...
static float nus_vdata[12][3] = {
	{-NUS_X, 0.0, NUS_Z}, {NUS_X, 0.0, NUS_Z}, {-NUS_X, 0.0, -NUS_Z}, {NUS_X, 0.0, -NUS_Z}, {0.0, NUS_Z, NUS_X}, {0.0, NUS_Z, -NUS_X}, {0.0, -NUS_Z, NUS_X}, {0.0, -NUS_Z, -NUS_X}, {NUS_Z, NUS_X, 0.0}, {-NUS_Z, NUS_X, 0.0}, {NUS_Z, -NUS_X, 0.0}, {-NUS_Z, -NUS_X, 0.0}};
...
void Nu_DrawPlanet(void **data)
{
	...
	glLightiv(GL_LIGHT1, GL_DIFFUSE, (const GLint *)light_col);
	glLightiv(GL_LIGHT1, GL_AMBIENT, (const GLint *)obj_col);
